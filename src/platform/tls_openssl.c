...
#pragma warning(push)
#pragma warning( \
    disable : 4100)  // Unreferenced parameter errcode in inline function
#endif
...
const size_t OpenSslFilePrefixLength =
    sizeof("..\\..\\..\\..\\..\\..\\submodules");
...
//
#define CXPLAT_TLS_DEFAULT_SSL_CIPHERS \
...
static QUIC_STATUS CxPlatTlsMapOpenSSLErrorToQuicStatus(_In_ int OpenSSLError) {
  switch (OpenSSLError) {
...
static int CxPlatTlsAlpnSelectCallback(
    _In_ SSL* Ssl, _Out_writes_bytes_(*OutLen) const unsigned char** Out,
    _Out_ unsigned char* OutLen,
    _In_reads_bytes_(InLen) const unsigned char* In, _In_ unsigned int InLen,
    _In_ void* Arg) {
...
static int CxPlatTlsCertificateVerifyCallback(X509_STORE_CTX* x509_ctx,
                                              void* param) {
  ...
  if ((TlsContext->SecConfig->Flags & QUIC_CREDENTIAL_FLAG_CLIENT ||
    ...
    if (!(TlsContext->SecConfig->Flags &
          QUIC_CREDENTIAL_FLAG_USE_TLS_BUILTIN_CERTIFICATE_VALIDATION)) {
      if (Cert == NULL) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.",
                       TlsContext->Connection, "No certificate passed");
      ...
      if (OpenSSLCertLength <= 0) {
        QuicTraceEvent(LibraryError, "[ lib] ERROR, %s.", "i2d_X509 failed");
        CertificateVerified = FALSE;
      } else {
        CertificateVerified = CxPlatCertVerifyRawCertificate(
            OpenSSLCertBuffer, OpenSSLCertLength, TlsContext->SNI,
            TlsContext->SecConfig->Flags,
            IsDeferredValidationOrClientAuth ? (uint32_t*)&ValidationResult
                                             : NULL);
      }
      ...
      if (IsDeferredValidationOrClientAuth && CertificateVerified <= 0) {
        ValidationResult = (int)CxPlatTlsMapOpenSSLErrorToQuicStatus(
            X509_STORE_CTX_get_error(x509_ctx));
      }
    }
  } else if ((TlsContext->SecConfig->Flags &
              QUIC_CREDENTIAL_FLAG_INDICATE_CERTIFICATE_RECEIVED) &&
             (TlsContext->SecConfig->Flags &
              QUIC_CREDENTIAL_FLAG_USE_PORTABLE_CERTIFICATES)) {
    //
    // We need to get certificates provided by peer if we going to pass them via
    // Callbacks.CertificateReceived. We don't really care about validation
    // status but without calling X509_verify_cert() x509_ctx has no
    // certificates attached to it and that impacts validation of custom
    // certificate chains.
    //
  ...
  if (!(TlsContext->SecConfig->Flags &
      ...
      !(TlsContext->SecConfig->Flags &
        QUIC_CREDENTIAL_FLAG_DEFER_CERTIFICATE_VALIDATION) &&
      !CertificateVerified) {
    QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection,
  ...
  if ((TlsContext->SecConfig->Flags &
      ...
      !TlsContext->SecConfig->Callbacks.CertificateReceived(
          ...
          (TlsContext->SecConfig->Flags &
           QUIC_CREDENTIAL_FLAG_USE_PORTABLE_CERTIFICATES)
              ? (QUIC_CERTIFICATE*)&PortableCertificate
              : (QUIC_CERTIFICATE*)Cert,
          (TlsContext->SecConfig->Flags &
           QUIC_CREDENTIAL_FLAG_USE_PORTABLE_CERTIFICATES)
              ? (QUIC_CERTIFICATE_CHAIN*)&PortableChain
              : (QUIC_CERTIFICATE_CHAIN*)x509_ctx,
          0, ValidationResult)) {
    QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection,
...
CXPLAT_STATIC_ASSERT((int)ssl_encryption_initial ==
                         (int)QUIC_PACKET_KEY_INITIAL,
                     "Code assumes exact match!");
CXPLAT_STATIC_ASSERT((int)ssl_encryption_early_data ==
                         (int)QUIC_PACKET_KEY_0_RTT,
                     "Code assumes exact match!");
CXPLAT_STATIC_ASSERT((int)ssl_encryption_handshake ==
                         (int)QUIC_PACKET_KEY_HANDSHAKE,
                     "Code assumes exact match!");
CXPLAT_STATIC_ASSERT((int)ssl_encryption_application ==
                         (int)QUIC_PACKET_KEY_1_RTT,
                     "Code assumes exact match!");

void CxPlatTlsNegotiatedCiphers(_In_ CXPLAT_TLS* TlsContext,
...
int CxPlatTlsSetEncryptionSecretsCallback(
    _In_ SSL* Ssl, _In_ OSSL_ENCRYPTION_LEVEL Level,
    _In_reads_(SecretLen) const uint8_t* ReadSecret,
    _In_reads_(SecretLen) const uint8_t* WriteSecret, _In_ size_t SecretLen) {
  CXPLAT_TLS* TlsContext = SSL_get_app_data(Ssl);
  CXPLAT_TLS_PROCESS_STATE* TlsState = TlsContext->State;
  QUIC_PACKET_KEY_TYPE KeyType = (QUIC_PACKET_KEY_TYPE)Level;
  QUIC_STATUS Status;

  QuicTraceLogConnVerbose(OpenSslNewEncryptionSecrets, TlsContext->Connection,
  ...
  if (ReadSecret) {
    ...
    Status =
        QuicPacketKeyDerive(KeyType, TlsContext->HkdfLabels, &Secret,
                            "read secret", TRUE, &TlsState->ReadKeys[KeyType]);
    if (QUIC_FAILED(Status)) {
      TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
      return -1;
    }
  ...
  if (TlsContext->TlsSecrets != NULL) {
    ...
    switch (KeyType) {
      ...
      case QUIC_PACKET_KEY_1_RTT:
        CXPLAT_FRE_ASSERT(ReadSecret != NULL && WriteSecret != NULL);
        if (TlsContext->IsServer) {
          memcpy(TlsContext->TlsSecrets->ClientTrafficSecret0, ReadSecret,
                 SecretLen);
          memcpy(TlsContext->TlsSecrets->ServerTrafficSecret0, WriteSecret,
                 SecretLen);
        } else {
          memcpy(TlsContext->TlsSecrets->ClientTrafficSecret0, WriteSecret,
                 SecretLen);
          memcpy(TlsContext->TlsSecrets->ServerTrafficSecret0, ReadSecret,
                 SecretLen);
        }
      ...
      case QUIC_PACKET_KEY_0_RTT:
        if (!TlsContext->IsServer) {
          ...
          memcpy(TlsContext->TlsSecrets->ClientEarlyTrafficSecret, WriteSecret,
                 SecretLen);
...
int CxPlatTlsAddHandshakeDataCallback(_In_ SSL* Ssl,
  ...
  QuicTraceLogConnVerbose(OpenSslAddHandshakeData, TlsContext->Connection,
                          "Sending %llu handshake bytes (Level = %u)",
                          (uint64_t)Length, (uint32_t)Level);
  ...
  if (Length + TlsState->BufferLength > (size_t)TlsState->BufferAllocLength) {
    ...
    uint8_t* NewBuffer =
        CXPLAT_ALLOC_NONPAGED(NewBufferAllocLength, QUIC_POOL_TLS_BUFFER);
    if (NewBuffer == NULL) {
      QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)",
                     "New crypto buffer", NewBufferAllocLength);
      TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
      return -1;
    }

    CxPlatCopyMemory(NewBuffer, TlsState->Buffer, TlsState->BufferLength);
  ...
  switch (KeyType) {
    case QUIC_PACKET_KEY_HANDSHAKE:
      if (TlsState->BufferOffsetHandshake == 0) {
        ...
        QuicTraceLogConnInfo(OpenSslHandshakeDataStart, TlsContext->Connection,
    ...
    case QUIC_PACKET_KEY_1_RTT:
      if (TlsState->BufferOffset1Rtt == 0) {
        ...
        QuicTraceLogConnInfo(OpenSsl1RttDataStart, TlsContext->Connection,
  ...
  CxPlatCopyMemory(TlsState->Buffer + TlsState->BufferLength, Data, Length);
...
int CxPlatTlsFlushFlightCallback(_In_ SSL* Ssl) {
  UNREFERENCED_PARAMETER(Ssl);
  return 1;
}

int CxPlatTlsSendAlertCallback(_In_ SSL* Ssl,
  ...
  QuicTraceLogConnError(OpenSslAlert, TlsContext->Connection,
                        "Send alert = %u (Level = %u)", Alert, (uint32_t)Level);
...
_Success_(return == SSL_CLIENT_HELLO_SUCCESS) int CxPlatTlsClientHelloCallback(
    _In_ SSL* Ssl, _When_(return == SSL_CLIENT_HELLO_ERROR, _Out_) int* Alert,
    _In_ void* arg) {
  UNREFERENCED_PARAMETER(arg);
  CXPLAT_TLS* TlsContext = SSL_get_app_data(Ssl);
  ...
  if (!SSL_client_hello_get0_ext(Ssl, TlsContext->QuicTpExtType,
                                 &TransportParams, &TransportParamLen)) {
    TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
...
int CxPlatTlsOnClientSessionTicketReceived(_In_ SSL* Ssl,
  ...
  if (Bio) {
    if (PEM_write_bio_SSL_SESSION(Bio, Session) == 1) {
      ...
      if (Length < UINT16_MAX) {
        QuicTraceLogConnInfo(OpenSslOnRecvTicket, TlsContext->Connection,
                             "Received session ticket, %u bytes",
                             (uint32_t)Length);
        TlsContext->SecConfig->Callbacks.ReceiveTicket(TlsContext->Connection,
                                                       (uint32_t)Length, Data);
      } else {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.",
                       TlsContext->Connection, "Session data too big");
      }
    } else {
      QuicTraceEvent(TlsErrorStatus, "[ tls][%p] ERROR, %u, %s.",
                     TlsContext->Connection, ERR_get_error(),
...
_Success_(return > 0) int CxPlatTlsOnSessionTicketKeyNeeded(
    _In_ SSL* Ssl,
    _When_(enc, _Out_writes_bytes_(16))
        _When_(!enc, _In_reads_bytes_(16)) unsigned char key_name[16],
    _When_(enc, _Out_writes_bytes_(EVP_MAX_IV_LENGTH))
        _When_(!enc, _In_reads_bytes_(EVP_MAX_IV_LENGTH)) unsigned char
  ...
  CXPLAT_STATIC_ASSERT(sizeof(TicketKey->Id) == 16,
  ...
  if (enc) {
    ...
    EVP_EncryptInit_ex(ctx, CXPLAT_AES_256_CBC_ALG_HANDLE, NULL,
                       TicketKey->Material, iv);

#ifdef IS_OPENSSL_3
    params[0] = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY,
                                                  TicketKey->Material, 32);
    ...
    HMAC_Init_ex(hctx, TicketKey->Material, 32, EVP_sha256(), NULL);
#endif
    EVP_DecryptInit_ex(ctx, CXPLAT_AES_256_CBC_ALG_HANDLE, NULL,
                       TicketKey->Material, iv);
  }
...
int CxPlatTlsOnServerSessionTicketGenerated(_In_ SSL* Ssl, _In_ void* arg) {
  CXPLAT_TLS* TlsContext = SSL_get_app_data(Ssl);
...
CxPlatTlsOnServerSessionTicketDecrypted(
    _In_ SSL* Ssl, _In_ SSL_SESSION* Session, _In_ const unsigned char* keyname,
    _In_ size_t keyname_length, _In_ SSL_TICKET_STATUS status, _In_ void* arg) {
  CXPLAT_TLS* TlsContext = SSL_get_app_data(Ssl);
  UNREFERENCED_PARAMETER(keyname);
  UNREFERENCED_PARAMETER(keyname_length);
  UNREFERENCED_PARAMETER(arg);

  QuicTraceLogConnVerbose(OpenSslTickedDecrypted, TlsContext->Connection,
  ...
  if (Session != NULL &&
    ...
    QuicTraceLogConnVerbose(OpenSslRecvTicketData, TlsContext->Connection,
                            "Received ticket data, %u bytes", (uint32_t)Length);

    if (!TlsContext->SecConfig->Callbacks.ReceiveTicket(
            TlsContext->Connection, (uint32_t)Length, Buffer)) {
      QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection,
...
SSL_QUIC_METHOD OpenSslQuicCallbacks = {
    CxPlatTlsSetEncryptionSecretsCallback, CxPlatTlsAddHandshakeDataCallback,
    CxPlatTlsFlushFlightCallback, CxPlatTlsSendAlertCallback};

CXPLAT_STATIC_ASSERT(FIELD_OFFSET(QUIC_CERTIFICATE_FILE, PrivateKeyFile) ==
                         FIELD_OFFSET(QUIC_CERTIFICATE_FILE_PROTECTED,
                                      PrivateKeyFile),
                     "Mismatch (private key) in certificate file structs");

CXPLAT_STATIC_ASSERT(FIELD_OFFSET(QUIC_CERTIFICATE_FILE, CertificateFile) ==
                         FIELD_OFFSET(QUIC_CERTIFICATE_FILE_PROTECTED,
                                      CertificateFile),
                     "Mismatch (certificate file) in certificate file structs");

_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_TLS_PROVIDER
    CxPlatTlsGetProvider(void) {
  return QUIC_TLS_PROVIDER_OPENSSL;
}

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatTlsSecConfigCreate(
    _In_ const QUIC_CREDENTIAL_CONFIG* CredConfig,
    _In_ CXPLAT_TLS_CREDENTIAL_FLAGS TlsCredFlags,
    _In_ const CXPLAT_TLS_CALLBACKS* TlsCallbacks, _In_opt_ void* Context,
  ...
  if (CredConfigFlags & QUIC_CREDENTIAL_FLAG_ENABLE_OCSP ||
    ...
    return QUIC_STATUS_NOT_SUPPORTED;  // Not supported by this TLS
                                       // implementation
  }
  ...
  if ((CredConfigFlags & QUIC_CREDENTIAL_FLAG_DEFER_CERTIFICATE_VALIDATION) &&
    ...
    return QUIC_STATUS_INVALID_PARAMETER;  // Defer validation without
                                           // indication doesn't make sense.
  }
  ...
  if (((CredConfigFlags &
        QUIC_CREDENTIAL_FLAG_USE_TLS_BUILTIN_CERTIFICATE_VALIDATION) == 0) &&
      (CredConfigFlags & QUIC_CREDENTIAL_FLAG_REVOCATION_CHECK_END_CERT ||
       CredConfigFlags &
           QUIC_CREDENTIAL_FLAG_REVOCATION_CHECK_CHAIN_EXCLUDE_ROOT ||
       CredConfigFlags & QUIC_CREDENTIAL_FLAG_IGNORE_NO_REVOCATION_CHECK ||
  ...
  if (CredConfig->Reserved != NULL) {
    return QUIC_STATUS_INVALID_PARAMETER;  // Not currently used and should be
                                           // NULL.
  }
  ...
  if (CredConfig->Type == QUIC_CREDENTIAL_TYPE_CERTIFICATE_FILE) {
    ...
    if (CredConfig->CertificatePkcs12 == NULL ||
      ...
      CredConfig->Type ==
          QUIC_CREDENTIAL_TYPE_CERTIFICATE_CONTEXT) {  // NOLINT
  ...
  if (CredConfigFlags & QUIC_CREDENTIAL_FLAG_SET_ALLOWED_CIPHER_SUITES &&
    ...
    QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                   CredConfig->AllowedCipherSuites,
  ...
  SecurityConfig =
      CXPLAT_ALLOC_NONPAGED(sizeof(CXPLAT_SEC_CONFIG), QUIC_POOL_TLS_SECCONF);
  if (SecurityConfig == NULL) {
    QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)",
                   "CXPLAT_SEC_CONFIG", sizeof(CXPLAT_SEC_CONFIG));
  ...
  if (SecurityConfig->SSLCtx == NULL) {
    QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", ERR_get_error(),
                   "SSL_CTX_new failed");
  ...
  if (CredConfigFlags & QUIC_CREDENTIAL_FLAG_SET_ALLOWED_CIPHER_SUITES) {
    ...
    if (CredConfig->AllowedCipherSuites &
      ...
      CipherSuiteStringLength +=
          (uint8_t)sizeof(CXPLAT_TLS_CHACHA20_POLY1305_SHA256);
      AllowedCipherSuitesCount++;
    }

    CipherSuiteString = CXPLAT_ALLOC_NONPAGED(
        CipherSuiteStringLength, QUIC_POOL_TLS_CIPHER_SUITE_STRING);
    if (CipherSuiteString == NULL) {
      QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)",
                     "CipherSuiteString", CipherSuiteStringLength);
      Status = QUIC_STATUS_OUT_OF_MEMORY;
      goto Exit;
    }
    ...
    if (CredConfig->AllowedCipherSuites &
        QUIC_ALLOWED_CIPHER_SUITE_AES_256_GCM_SHA384) {
      CxPlatCopyMemory(&CipherSuiteString[CipherSuiteStringCursor],
  ...
  if ((CredConfigFlags & QUIC_CREDENTIAL_FLAG_CLIENT) &&
    ...
    SSL_CTX_sess_set_new_cb(SecurityConfig->SSLCtx,
  ...
  if (!(CredConfigFlags & QUIC_CREDENTIAL_FLAG_CLIENT)) {
    if (!(TlsCredFlags & CXPLAT_TLS_CREDENTIAL_FLAG_DISABLE_RESUMPTION)) {
      ...
      if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                       ERR_get_error(), "SSL_CTX_set_max_early_data failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
      }

      Ret = SSL_CTX_set_session_ticket_cb(
          SecurityConfig->SSLCtx, NULL, CxPlatTlsOnServerSessionTicketDecrypted,
          NULL);
      if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                       ERR_get_error(), "SSL_CTX_set_session_ticket_cb failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
      }
    }
    ...
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(), "SSL_CTX_set_num_tickets failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
  }
  ...
  if (CredConfig->Type == QUIC_CREDENTIAL_TYPE_CERTIFICATE_FILE ||
    ...
    if (CredConfig->Type == QUIC_CREDENTIAL_TYPE_CERTIFICATE_FILE_PROTECTED) {
      SSL_CTX_set_default_passwd_cb_userdata(
          SecurityConfig->SSLCtx,
          (void*)CredConfig->CertificateFileProtected->PrivateKeyPassword);
    }

    Ret = SSL_CTX_use_PrivateKey_file(
        SecurityConfig->SSLCtx, CredConfig->CertificateFile->PrivateKeyFile,
        SSL_FILETYPE_PEM);
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(), "SSL_CTX_use_PrivateKey_file failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }

    Ret = SSL_CTX_use_certificate_chain_file(
        SecurityConfig->SSLCtx, CredConfig->CertificateFile->CertificateFile);
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(),
    ...
    if (CredConfig->Type == QUIC_CREDENTIAL_TYPE_CERTIFICATE_PKCS12) {
      ...
      Ret = BIO_write(Bio, CredConfig->CertificatePkcs12->Asn1Blob,
                      CredConfig->CertificatePkcs12->Asn1BlobLength);
      if (Ret < 0) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                       ERR_get_error(), "BIO_write failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
      }
    } else {
      ...
      Status = CxPlatCertExtractPrivateKey(CredConfig, PasswordBuffer, &PfxBlob,
                                           &PfxSize);
      if (QUIC_FAILED(Status)) {
        goto Exit;
      }
    ...
    if (!Pkcs12) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(), "d2i_PKCS12_bio failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
    ...
    if (CaCertificates) {
      ...
      while ((CaCert = sk_X509_pop(CaCertificates)) != NULL) {
        //
        // This transfers ownership to SSLCtx and CaCert does not need to be
        // freed.
        //
    ...
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(), "PKCS12_parse failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
    ...
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(), "SSL_CTX_use_certificate failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
  }
  ...
  if (CredConfig->Type != QUIC_CREDENTIAL_TYPE_NONE) {
    ...
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.",
                     ERR_get_error(), "SSL_CTX_check_private_key failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
  }
  ...
  if (CredConfigFlags & QUIC_CREDENTIAL_FLAG_CLIENT) {
    SSL_CTX_set_cert_verify_callback(SecurityConfig->SSLCtx,
                                     CxPlatTlsCertificateVerifyCallback, NULL);
    SSL_CTX_set_verify(SecurityConfig->SSLCtx, SSL_VERIFY_PEER, NULL);
    if (!(CredConfigFlags &
          (QUIC_CREDENTIAL_FLAG_NO_CERTIFICATE_VALIDATION |
           QUIC_CREDENTIAL_FLAG_DEFER_CERTIFICATE_VALIDATION))) {
      SSL_CTX_set_verify_depth(SecurityConfig->SSLCtx,
    ...
    SSL_CTX_set_options(SecurityConfig->SSLCtx,
                        (SSL_OP_ALL & ~SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS) |
                            SSL_OP_SINGLE_ECDH_USE |
                            SSL_OP_CIPHER_SERVER_PREFERENCE |
                            SSL_OP_NO_ANTI_REPLAY);
    SSL_CTX_clear_options(SecurityConfig->SSLCtx,
    ...
    if (CredConfigFlags & QUIC_CREDENTIAL_FLAG_INDICATE_CERTIFICATE_RECEIVED ||
      ...
      SSL_CTX_set_cert_verify_callback(
          SecurityConfig->SSLCtx, CxPlatTlsCertificateVerifyCallback, NULL);
    }
    ...
    if (CredConfigFlags & QUIC_CREDENTIAL_FLAG_REQUIRE_CLIENT_AUTHENTICATION) {
      ...
      SSL_CTX_set_verify(SecurityConfig->SSLCtx, VerifyMode, NULL);
    }

    SSL_CTX_set_alpn_select_cb(SecurityConfig->SSLCtx,
                               CxPlatTlsAlpnSelectCallback, NULL);

    SSL_CTX_set_max_early_data(SecurityConfig->SSLCtx, UINT32_MAX);
    SSL_CTX_set_client_hello_cb(SecurityConfig->SSLCtx,
                                CxPlatTlsClientHelloCallback, NULL);
  }
...
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatTlsSecConfigDelete(__drv_freesMem(
    ServerConfig) _Frees_ptr_ _In_ CXPLAT_SEC_CONFIG* SecurityConfig) {
  if (SecurityConfig->SSLCtx != NULL) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatTlsSecConfigSetTicketKeys(_In_ CXPLAT_SEC_CONFIG* SecurityConfig,
                                    _In_reads_(KeyCount)
  ...
  if (SecurityConfig->TicketKey == NULL) {
    SecurityConfig->TicketKey = CXPLAT_ALLOC_NONPAGED(
        sizeof(QUIC_TICKET_KEY_CONFIG), QUIC_POOL_TLS_TICKET_KEY);
    if (SecurityConfig->TicketKey == NULL) {
      QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)",
                     "QUIC_TICKET_KEY_CONFIG", sizeof(QUIC_TICKET_KEY_CONFIG));
      return QUIC_STATUS_OUT_OF_MEMORY;
    }
  }

  CxPlatCopyMemory(SecurityConfig->TicketKey, KeyConfig,
                   sizeof(QUIC_TICKET_KEY_CONFIG));

#ifdef IS_OPENSSL_3
  SSL_CTX_set_tlsext_ticket_key_evp_cb(SecurityConfig->SSLCtx,
                                       CxPlatTlsOnSessionTicketKeyNeeded);
#else
  SSL_CTX_set_tlsext_ticket_key_cb(SecurityConfig->SSLCtx,
                                   CxPlatTlsOnSessionTicketKeyNeeded);
#endif
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatTlsInitialize(_In_ const CXPLAT_TLS_CONFIG* Config,
  ...
  QuicTraceLogConnVerbose(OpenSslContextCreated, TlsContext->Connection,
                          "TLS context Created");

  if (!Config->IsServer) {
    if (Config->ServerName != NULL) {
      ServerNameLength =
          (uint16_t)strnlen(Config->ServerName, QUIC_MAX_SNI_LENGTH);
      if (ServerNameLength == QUIC_MAX_SNI_LENGTH) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.",
                       TlsContext->Connection, "SNI Too Long");
  ...
  if (Config->IsServer) {
    ...
    SSL_set_alpn_protos(TlsContext->Ssl, TlsContext->AlpnBuffer,
                        TlsContext->AlpnBufferLength);
  }

  if (!(Config->SecConfig->TlsFlags &
    ...
    if (Config->ResumptionTicketLength != 0) {
      ...
      QuicTraceLogConnInfo(OpenSslOnSetTicket, TlsContext->Connection,
                           "Setting session ticket, %u bytes",
                           Config->ResumptionTicketLength);
      BIO* Bio = BIO_new_mem_buf(Config->ResumptionTicketBuffer,
  ...
  SSL_set_quic_use_legacy_codepoint(
      TlsContext->Ssl, TlsContext->QuicTpExtType ==
                           TLS_EXTENSION_TYPE_QUIC_TRANSPORT_PARAMETERS_DRAFT);

  if (SSL_set_quic_transport_params(TlsContext->Ssl, Config->LocalTPBuffer,
  ...
  if (Config->ResumptionTicketBuffer) {
    CXPLAT_FREE(Config->ResumptionTicketBuffer,
                QUIC_POOL_CRYPTO_RESUMPTION_TICKET);
  }
...
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatTlsUninitialize(
    _In_opt_ CXPLAT_TLS* TlsContext) {
  if (TlsContext != NULL) {
    QuicTraceLogConnVerbose(OpenSslContextCleaningUp, TlsContext->Connection,
                            "Cleaning up");
...
_IRQL_requires_max_(PASSIVE_LEVEL) void CxPlatTlsUpdateHkdfLabels(
    _In_ CXPLAT_TLS* TlsContext, _In_ const QUIC_HKDF_LABELS* const Labels) {
  TlsContext->HkdfLabels = Labels;
}

_IRQL_requires_max_(PASSIVE_LEVEL) CXPLAT_TLS_RESULT_FLAGS
    CxPlatTlsProcessData(_In_ CXPLAT_TLS* TlsContext,
  ...
  if (DataType == CXPLAT_TLS_TICKET_DATA) {
    QuicTraceLogConnVerbose(OpenSslSendTicketData, TlsContext->Connection,
                            "Sending ticket data, %u bytes", *BufferLength);
    ...
    if (Ret != 1) {
      QuicTraceEvent(
          TlsErrorStatus, "[ tls][%p] ERROR, %u, %s.", TlsContext->Connection,
          SSL_get_error(TlsContext->Ssl, Ret), "SSL_do_handshake failed");
      TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
      goto Exit;
    }
  ...
  if (*BufferLength != 0) {
    QuicTraceLogConnVerbose(OpenSslProcessData, TlsContext->Connection,
                            "Processing %u received bytes", *BufferLength);

    if (SSL_provide_quic_data(TlsContext->Ssl,
                              (OSSL_ENCRYPTION_LEVEL)TlsContext->State->ReadKey,
                              Buffer, *BufferLength) != 1) {
      char buf[256];
      QuicTraceLogConnError(OpenSslQuicDataErrorStr, TlsContext->Connection,
  ...
  if (!State->HandshakeComplete) {
    ...
    if (Ret <= 0) {
      ...
      switch (Err) {
        ...
        case SSL_ERROR_WANT_WRITE:
          //
          // Best effort to get the server's transport params as early as
          // possible.
          //
          if (!TlsContext->IsServer && TlsContext->PeerTPReceived == FALSE) {
            ...
            if (TransportParams != NULL && TransportParamLen != 0) {
              ...
              if (!TlsContext->SecConfig->Callbacks.ReceiveTP(
                      TlsContext->Connection, (uint16_t)TransportParamLen,
                      TransportParams)) {
                TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
                goto Exit;
              }
        ...
        case SSL_ERROR_SSL: {
          ...
          int line;
#ifdef IS_OPENSSL_3
          ERR_error_string_n(ERR_get_error_all(&file, &line, NULL, NULL, NULL),
                             buf, sizeof(buf));
#else
          ERR_error_string_n(ERR_get_error_line(&file, &line), buf,
                             sizeof(buf));
#endif
          QuicTraceLogConnError(OpenSslHandshakeErrorStr,
                                TlsContext->Connection,
                                "TLS handshake error: %s, file:%s:%d", buf,
                                (strlen(file) > OpenSslFilePrefixLength
        ...
        default:
          QuicTraceLogConnError(OpenSslHandshakeError, TlsContext->Connection,
                                "TLS handshake error: %d", Err);
          TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
          goto Exit;
      }
    }
    ...
    if (!TlsContext->IsServer) {
      ...
      SSL_get0_alpn_selected(TlsContext->Ssl, &NegotiatedAlpn,
                             &NegotiatedAlpnLength);
      if (NegotiatedAlpnLength == 0) {
        QuicTraceLogConnError(OpenSslAlpnNegotiationFailure,
                              TlsContext->Connection,
      ...
      if (NegotiatedAlpnLength > UINT8_MAX) {
        QuicTraceLogConnError(OpenSslInvalidAlpnLength, TlsContext->Connection,
                              "Invalid negotiated ALPN length");
        TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
        goto Exit;
      }
      TlsContext->State->NegotiatedAlpn = CxPlatTlsAlpnFindInList(
          TlsContext->AlpnBufferLength, TlsContext->AlpnBuffer,
          (uint8_t)NegotiatedAlpnLength, NegotiatedAlpn);
      if (TlsContext->State->NegotiatedAlpn == NULL) {
        QuicTraceLogConnError(OpenSslNoMatchingAlpn, TlsContext->Connection,
      ...
      if (!TlsContext->SecConfig->Callbacks.CertificateReceived(
              TlsContext->Connection, NULL, NULL, 0, ValidationResult)) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.",
                       TlsContext->Connection,
                       "Indicate null certificate received failed");
        TlsContext->ResultFlags |= CXPLAT_TLS_RESULT_ERROR;
        TlsContext->State->AlertCode =
            CXPLAT_TLS_ALERT_CODE_REQUIRED_CERTIFICATE;
        goto Exit;
      }
    }

    QuicTraceLogConnInfo(OpenSslHandshakeComplete, TlsContext->Connection,
                         "TLS Handshake complete");
    State->HandshakeComplete = TRUE;
    if (SSL_session_reused(TlsContext->Ssl)) {
      QuicTraceLogConnInfo(OpenSslHandshakeResumed, TlsContext->Connection,
    ...
    if (TlsContext->IsServer) {
      ...
      const uint8_t* TransportParams;
      size_t TransportParamLen;
      SSL_get_peer_quic_transport_params(TlsContext->Ssl, &TransportParams,
                                         &TransportParamLen);
      if (TransportParams == NULL || TransportParamLen == 0) {
        QuicTraceLogConnError(OpenSslMissingTransportParameters,
                              TlsContext->Connection,
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatSecConfigParamSet(_In_ CXPLAT_SEC_CONFIG* TlsContext,
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatSecConfigParamGet(
    _In_ CXPLAT_SEC_CONFIG* SecConfig, _In_ uint32_t Param,
    _Inout_ uint32_t* BufferLength,
    _Inout_updates_bytes_opt_(*BufferLength) void* Buffer) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatTlsParamSet(_In_ CXPLAT_TLS* SecConfig, _In_ uint32_t Param,
...
static QUIC_STATUS CxPlatMapCipherSuite(
...
static uint32_t CxPlatMapVersion(_In_z_ const char* Version) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatTlsParamGet(_In_ CXPLAT_TLS* TlsContext, _In_ uint32_t Param,
                      _Inout_ uint32_t* BufferLength,
                      _Inout_updates_bytes_opt_(*BufferLength) void* Buffer) {
  QUIC_STATUS Status;
  ...
  switch (Param) {
    case QUIC_PARAM_TLS_HANDSHAKE_INFO: {
      ...
      HandshakeInfo->TlsProtocolVersion =
          CxPlatMapVersion(SSL_get_version(TlsContext->Ssl));

      const SSL_CIPHER* Cipher = SSL_get_current_cipher(TlsContext->Ssl);
      if (Cipher == NULL) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.",
                       TlsContext->Connection, "Unable to get cipher suite");
    ...
    case QUIC_PARAM_TLS_NEGOTIATED_ALPN: {
      ...
      if (NegotiatedAlpnLen == 0) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.",
                       TlsContext->Connection, "Unable to get negotiated alpn");
        Status = QUIC_STATUS_INVALID_STATE;
        break;
      }
