...
#define TLS_EXTENSION_TYPE_APPLICATION_LAYER_PROTOCOL_NEGOTIATION \
  0x0010  // Host Byte Order
#define TLS_EXTENSION_TYPE_QUIC_TRANSPORT_PARAMETERS_DRAFT \
  0xffa5                                                     // Host Byte Order
...
//
typedef _IRQL_requires_max_(DISPATCH_LEVEL) void(
    CXPLAT_TLS_PROCESS_COMPLETE_CALLBACK)(_In_ QUIC_CONNECTION* Connection);

typedef CXPLAT_TLS_PROCESS_COMPLETE_CALLBACK*
    CXPLAT_TLS_PROCESS_COMPLETE_CALLBACK_HANDLER;
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    BOOLEAN(CXPLAT_TLS_RECEIVE_TP_CALLBACK)(_In_ QUIC_CONNECTION* Connection,
                                            _In_ uint16_t TPLength,
                                            _In_reads_(TPLength)
                                                const uint8_t* TPBuffer);
...
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    BOOLEAN(CXPLAT_TLS_RECEIVE_TICKET_CALLBACK)(
        _In_ QUIC_CONNECTION* Connection, _In_ uint32_t TicketLength,
        _In_reads_(TicketLength) const uint8_t* Ticket);

typedef CXPLAT_TLS_RECEIVE_TICKET_CALLBACK*
    CXPLAT_TLS_RECEIVE_TICKET_CALLBACK_HANDLER;

//
// Callback for indicating the peer certificate is ready for custom validation.
//
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    BOOLEAN(CXPLAT_TLS_PEER_CERTIFICATE_RECEIVED_CALLBACK)(
        _In_ QUIC_CONNECTION* Connection,
        _In_opt_ QUIC_CERTIFICATE* Certificate,
        _In_opt_ QUIC_CERTIFICATE_CHAIN* Chain,
        _In_ uint32_t DeferredErrorFlags, _In_ QUIC_STATUS DeferredStatus);

typedef CXPLAT_TLS_PEER_CERTIFICATE_RECEIVED_CALLBACK*
    CXPLAT_TLS_PEER_CERTIFICATE_RECEIVED_CALLBACK_HANDLER;
...
typedef enum CXPLAT_TLS_RESULT_FLAGS {
  ...
  CXPLAT_TLS_RESULT_CONTINUE =
      0x0001,  // Needs immediate call again. (Used internally to schannel)
  CXPLAT_TLS_RESULT_DATA = 0x0002,  // Data ready to be sent.
  CXPLAT_TLS_RESULT_READ_KEY_UPDATED =
      0x0004,  // ReadKey variable has been updated.
  CXPLAT_TLS_RESULT_WRITE_KEY_UPDATED =
      0x0008,  // WriteKey variable has been updated.
  CXPLAT_TLS_RESULT_EARLY_DATA_ACCEPT =
      0x0010,  // The server accepted the early (0-RTT) data.
  CXPLAT_TLS_RESULT_EARLY_DATA_REJECT =
      0x0020,  // The server rejected the early (0-RTT) data.
...
typedef _IRQL_requires_max_(PASSIVE_LEVEL)
    _Function_class_(CXPLAT_SEC_CONFIG_CREATE_COMPLETE) void(
        QUIC_API CXPLAT_SEC_CONFIG_CREATE_COMPLETE)(
        _In_ const QUIC_CREDENTIAL_CONFIG* CredConfig, _In_opt_ void* Context,
        _In_ QUIC_STATUS Status, _In_opt_ CXPLAT_SEC_CONFIG* SecurityConfig);

typedef CXPLAT_SEC_CONFIG_CREATE_COMPLETE*
    CXPLAT_SEC_CONFIG_CREATE_COMPLETE_HANDLER;
...
// The returned CXPLAT_TLS_RESULT_FLAGS and CXPLAT_TLS_PROCESS_STATE are update
// with any state changes as a result of the call.
//
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS CxPlatSecConfigParamGet(
    _In_ CXPLAT_SEC_CONFIG* TlsContext, _In_ uint32_t Param,
    _Inout_ uint32_t* BufferLength,
...
//
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    CxPlatTlsParamSet(_In_ CXPLAT_TLS* TlsContext, _In_ uint32_t Param,
...
//
inline const uint8_t* CxPlatTlsAlpnFindInList(_In_ uint16_t AlpnListLength,
  ...
  std::cout << "[CxPlatTlsAlpnFindInList] Target: "
            << std::string((const char*)FindAlpn, FindAlpnLength) << std::endl;
  while (AlpnListLength != 0) {
    CXPLAT_DBG_ASSERT(AlpnList[0] + 1 <= AlpnListLength);

    std::cout << "[CxPlatTlsAlpnFindInList] Candidate: "
              << std::string((const char)(AlpnList + 1), FindAlpnLength)
              << std::endl;
